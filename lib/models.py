...
class GovernanceObject(BaseModel):
    ...
    @classmethod
    def sync(self, examinationd):
        golist = examinationd.rpc_command('gobject', 'list')
        ...
        try:
            ...
            for item in golist.values():
                (go, subobj) = self.import_gobject_from_examinationd(examinationd, item)
    ...
    @classmethod
    def import_gobject_from_examinationd(self, examinationd, rec):
        import decimal
        ...
        # shim/examinationd conversion
        object_hex = examinationlib.SHIM_deserialise_from_examinationd(object_hex)
        objects = examinationlib.deserialise(object_hex)
        subobj = None
        ...
        # exclude any invalid model data from examinationd...
        valid_keys = subclass.serialisable_fields()
        subdikt = {k: dikt[k] for k in valid_keys if k in dikt}

        # get/create, then sync vote counts from examinationd, with every run
        ...
        # get/create, then sync payment amounts, etc. from examinationd - Examinationcoind is the master
        try:
            newdikt = subdikt.copy()
            newdikt['object_hash'] = object_hash
            if subclass(**newdikt).is_valid() is False:
                govobj.vote_delete(examinationd)
                return (govobj, None)

            subobj, created = subclass.get_or_create(object_hash=object_hash, defaults=subdikt)
        except Exception as e:
            # in this case, vote as delete, and log the vote in the DB
            printdbg("Got invalid object from examinationd! %s" % e)
            govobj.vote_delete(examinationd)
            return (govobj, None)
    ...
    def vote_delete(self, examinationd):
        if not self.voted_on(signal=VoteSignals.delete, outcome=VoteOutcomes.yes):
            self.vote(examinationd, VoteSignals.delete, VoteOutcomes.yes)
        return
    ...
    def vote(self, examinationd, signal, outcome):
        import examinationlib
        ...
        output = examinationd.rpc_command(*vote_command)

        # extract vote output parsing to external lib
        voted = examinationlib.did_we_vote(output)
        ...
        if voted:
            ...
            self.sync_network_vote(examinationd, signal)

    def sync_network_vote(self, examinationd, signal):
        printdbg('\tsyncing network vote for object %s with signal %s' % (self.object_hash, signal.name))
        vote_info = examinationd.get_my_gobject_votes(self.object_hash)
...
class Proposal(GovernanceClass, BaseModel):
    ...
    def is_valid(self):
        ...
        try:
            ...
            # payment address is valid base58 examination addr, non-multisig
            if not examinationlib.is_valid_examination_address(self.payment_address, config.network):
                printdbg("\tPayment address [%s] not a valid Examinationcoin address for network [%s], returning False" % (self.payment_address, config.network))
                return False
    ...
    def is_expired(self, superblockcycle=None):
        ...
        expiration_window_seconds = int(
            (examinationlib.blocks_to_seconds(superblockcycle) / 2) +
    ...
    def is_deletable(self):
        ...
        # TBD (item moved to external storage/ExaminationcoinDrive, etc.)
        return False
    ...
    def get_prepare_command(self):
        import examinationlib
        obj_data = examinationlib.SHIM_serialise_for_examinationd(self.serialise())
    ...
    def prepare(self, examinationd):
        try:
            object_hash = examinationd.rpc_command(*self.get_prepare_command())
...
class Superblock(BaseModel, GovernanceClass):
    ...
    def is_valid(self):
        ...
        for addr in addresses:
            if not examinationlib.is_valid_examination_address(addr, config.network):
    ...
    def hash(self):
        import examinationlib
        return examinationlib.hashit(self.serialise())
...
class Watchdog(BaseModel, GovernanceClass):
    ...
    @classmethod
    def active(self, examinationd):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at >= (now - examinationd.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
        return resultset

    @classmethod
    def expired(self, examinationd):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at < (now - examinationd.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
    ...
    def is_expired(self, examinationd):
        now = int(time.time())
        return (self.created_at < (now - examinationd.SENTINEL_WATCHDOG_MAX_SECONDS))

    def is_valid(self, examinationd):
        if self.is_expired(examinationd):
            return False

        return True

    def is_deletable(self, examinationd):
        if self.is_expired(examinationd):
            return True
