...
class GovernanceClass(object):
    ...
    def vote(self, examinationd, signal, outcome):
        return self.go.vote(examinationd, signal, outcome)

    # pass thru to GovernanceObject#voted_on
    def voted_on(self, **kwargs):
        return self.go.voted_on(**kwargs)

    def vote_validity(self, examinationd):
        if self.is_valid():
            printdbg("Voting valid! %s: %d" % (self.__class__.__name__, self.id))
            self.vote(examinationd, models.VoteSignals.valid, models.VoteOutcomes.yes)
        else:
            printdbg("Voting INVALID! %s: %d" % (self.__class__.__name__, self.id))
            self.vote(examinationd, models.VoteSignals.valid, models.VoteOutcomes.no)
    ...
    def list(self):
        ...
        # return a dict similar to examinationd "gobject list" output
    ...
    def examinationd_serialise(self):
        import examinationlib
        return examinationlib.SHIM_serialise_for_examinationd(self.serialise())
