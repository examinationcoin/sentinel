...
def has_examination_conf():
    import config
    import io

    valid_examination_conf = False

    # ensure examination_conf exists & readable
    #
    # if not, print a message stating that Examinationcoin Core must be installed and
    # configured, including JSONRPC access in examination.conf
    try:
        f = io.open(config.examination_conf)
        valid_examination_conf = True
    except IOError as e:
        print(e)

    return valid_examination_conf
...
def main():
    ...
    if not has_examination_conf():
        print("ExaminationCoin must be installed and configured, including JSONRPC access in examination.conf")
        sys.exit(1)
