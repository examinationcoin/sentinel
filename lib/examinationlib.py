...
def is_valid_examination_address(address, network='mainnet'):
    ...
    examination_version = 140 if network == 'testnet' else 76
    ...
    try:
        ...
        # rescue from exception, not a valid Examinationcoin address
        return False

    if (address_version != examination_version):
        return False
...
# shims 'til we can fix the examinationd side
def SHIM_serialise_for_examinationd(sentinel_hex):
    from models import COIN2FLYD_GOVOBJ_TYPES
    # unpack
    obj = deserialise(sentinel_hex)

    # shim for examinationd
    govtype = obj[0]

    # add 'type' attribute
    obj[1]['type'] = COIN2FLYD_GOVOBJ_TYPES[govtype]

    # superblock => "trigger" in examinationd
    if govtype == 'superblock':
        obj[0] = 'trigger'

    # examinationd expects an array (even though there is only a 1:1 relationship between govobj->class)
    obj = [obj]

    # re-pack
    examinationd_hex = serialise(obj)
    return examinationd_hex


# shims 'til we can fix the examinationd side
def SHIM_deserialise_from_examinationd(examinationd_hex):
    from models import COIN2FLYD_GOVOBJ_TYPES

    # unpack
    obj = deserialise(examinationd_hex)

    # shim from examinationd
...
def did_we_vote(output):
    ...
    try:
        detail = output.get('detail').get('examination.conf')
