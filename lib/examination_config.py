...
class ExaminationcoinConfig():

    @classmethod
    def slurp_config_file(self, filename):
        # read examination.conf config but skip commented lines
        f = io.open(filename)
        lines = []
        for line in f:
            if re.match('^\s*#', line):
        ...
        # data is examination.conf without commented lines
        data = ''.join(lines)
    ...
    def get_rpc_creds(self, data, network='mainnet'):
        # get rpc info from examination.conf
        match = re.findall(r'rpc(user|password|port)=(.*?)$', data, re.MULTILINE)

        # python >= 2.7
        creds = {key: value.strip() for (key, value) in match}

        # standard Examinationcoin defaults...
        default_port = 7382 if (network == 'mainnet') else 22778

        # use default port for network if not specified in examination.conf
        if not ('port' in creds):
            creds[u'port'] = default_port

        # convert to an int if taken from examination.conf
