...
sentinel_cfg = ExaminationcoinConfig.tokenize(sentinel_config_file)
sentinel_version = "1.1.0"
min_examinationd_proto_version_with_sentinel_ping = 80000


def get_examination_conf():
    home = os.environ.get('HOME')

    examination_conf = os.path.join(home, ".examinationcore/examination.conf")
    if sys.platform == 'darwin':
        examination_conf = os.path.join(home, "Library/Application Support/ExaminationCoin/examination.conf")

    examination_conf = sentinel_cfg.get('examination_conf', examination_conf)

    return examination_conf
...
examination_conf = get_examination_conf()
network = get_network()
db = get_db_conn()
