"""
examinationd JSONRPC interface
"""
import sys
import os
...
class ExaminationcoinDaemon():
    def __init__(self, **kwargs):
        ...
        # memoize calls to some examinationd methods
    ...
    @classmethod
    def from_examination_conf(self, examination_dot_conf):
        from examination_config import ExaminationcoinConfig
        config_text = ExaminationcoinConfig.slurp_config_file(examination_dot_conf)
        creds = ExaminationcoinConfig.get_rpc_creds(config_text, config.network)
    ...
    def get_my_gobject_votes(self, object_hash):
        ...
        if not self.gobject_votes.get(object_hash):
            ...
            self.gobject_votes[object_hash] = examinationlib.parse_raw_votes(raw_votes)
    ...
    def we_are_the_winner(self):
        ...
        winner = examinationlib.elect_mn(block_hash=current_block_hash, mnlist=mn_list)
        my_vin = self.get_current_masternode_vin()
    ...
    def estimate_block_time(self, height):
        ...
        future_seconds = examinationlib.blocks_to_seconds(diff)
    ...
    def has_sentinel_ping(self):
        ...
        return False # (getinfo['protocolversion'] >= config.min_examinationd_proto_version_with_sentinel_ping)
